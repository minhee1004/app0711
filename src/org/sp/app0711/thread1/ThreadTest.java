...
/*
 * 1)쓰레드란? 
 * - 하나의 프로세스 (=실행중인 프로그램)내에서 독립적으로 실행될 수 있는 
 * 	세부 실행단위
 * 
 *  	                           OS                  jvm
 *    멀티태스킹대상         프로세스				쓰레드    
 ...
 *	 - 작성: 로직은 run() 메서드를 override  해야 된다.
 *	 - 생성: 개발자가 new 한다
 *  - Runnable 상태로 진입시킨다 : start() 호출
 *    jvm의 실행대상이 된다 즉 실행후보가 된다 
 *  - jvm 내부 알고리즘에 의해 랜덤하게 특정 Thread가 선택되고, 이 선택된
 *    쓰레드의 run() 메서드를 실행하는 단계를 Running 이라 한다.\
 *  - 소멸: run() 메서드의 닫는 } 브레이스를 만나면, 쓰레드를 소멸됨, 이때의 
 *  		생명주기를 상태를 가리켜 Dead라 한다.
 *  
...
public class ThreadTest extends JFrame{
	...
	public ThreadTest() {
		...
		//이벤트 연결 코드는 재사용성이 없기 때문에, 되도록이면 코드가 간소화 
		//될 수록 좋다 따라서 이벤트 구현시 내부익명클래스를 지원함.
		//java8 람다(Lambda) : 굳이 이벤트 연결 코드를 객체로까지 갈 필요있는가?
		//                              그냥 함수로 표현하는 방법을 지원함.
		//                              ex) js에서의 화살표함수랑 흡사
		...
		bt.addActionListener(new ActionListener() {
			//내부익명 클래스내의 메서드에서는 바깥쪽 외부 클래스(ThreadTest)의 
			//멤버변수를 내것처럼 쓸 수 있다. 
	...
	//왜 밭겨야 하나? 시분할로 멀티쓰레드 구현을 위함
	//메인실행부는 절대로, 대기상태나 무한루프에 빠트리면 안됨
	//왜? 메인쓰레드는 1) gui 에서 사용자의 이벤트를처리, 그래픽처리
	public void createThread() {
		...
		*/
		...
		//쓰레드의 실행완료를 기다리지 않는다. 
		//비동기방식이란? 특정 코드 끝날때까지 기다리지 않고, 다른 코드를 실행할 수 있는 방식
		System.out.println("메인쓰레드에 의해 실행");
	}
	
	public static void main(String[] args) {
		//아래의 코드를 실행하면, 에러 메시지에 main thread에서 에러가 발생
		//했다는 메시지가 출력된다.. 결론) 바자의 실행부라 불렀던 그 녀석은 사실
		//메인 쓰레드였다. 메인쓰레드는 프로그램을 운영하는 실행부이므로,
		//반드시 대기상태나, 로프에 빠트리지 말자!!(프로그램 먹통 됨)
		/*
		int[] arr= new int[2];
		arr[0] = 5;
		arr[1] = 7;
